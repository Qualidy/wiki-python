...
# Das Dictionary, dass als Argument übergeben wurde,
# bleibt aber unverändert.
# {'h': 1, 4: 2, 2.0: "e"} -> {'h': 1}
def remove_float_and_int_keys(dic):
    return {k: v for k, v in dic.items() if not isinstance(k, (int, float))}


# 7 Punkte
# Erstelle eine Funktion, bei einen String erwartet. Dieser
...
# Schreibe eine Funktion, die Reihenfolge alle Buchstaben zwischen
...
def keep_smallest_biggest(my_list):
    ...
    minimum = min(my_list)
    maximum = max(my_list)
    return [i for i in my_list if i in [maximum, minimum]]
...
def funny_sum(my_list):
    return sum(-n if n % 2 else n for n in my_list)
...
def is_left_bigger(my_list):
    return tuple(a > b for a, b in zip(my_list, my_list[1:]))
...
def all_len(words):
    return tuple(len(word) for word in words.split())
...
# und eine Liste in ein Tupel. Bei einem Set wird eine Liste
...
def change_type(my_obj):
    ...
    if isinstance(my_obj, set):
        return [min(my_obj), max(my_obj)] if len(my_obj) else []
...
#
# Angenommen, es existiert eine Funktion add(a,b), die die Summe
# Ihrer beiden Parameter zurückgibt.
# apply_operation([(1, 3), (4.5, 5.5)], add) -> [4, 10.0]
def apply_operation(list_of_pairs, operation):
    return [operation(a, b) for a, b in list_of_pairs]
...
def deep_positive_check(list_of_lists, zero_is_true=True):
    return [[zero_is_true if element == 0 else element > 0 for element in sublist] for sublist in list_of_lists]
