...
# Schreibe eine Funktion, welche die Reihenfolge aller Buchstaben zwischen
...
def resort_within_word(words):
    ...
    if isinstance(words,str):
        wortliste=words.split(" ")
        zaehler=0
        for wort in wortliste:
            reversed=""
            for i in range(len(wort)):
                reversed+=wort[len(wort)-(i+1)]
            if zaehler:
                newWords += " "+ reversed
            else:
                newWords =reversed
    ...
    for i in range(len(my_list)):
        if i<(len(my_list)-1):
            if my_list[i]<=my_list[i+1]:
                ...
                tuple1.append(True)
    return tuple(tuple1) # Von Viktor Korrigiert
...
def swap_digets(number:int):
    revesed=str(number)
    rueckgabe=str
    for i in range(len(revesed)):
        rueckgabe+=revesed[len(revesed)-(i+1)]
    return rueckgabe # Von viktor korrigiert
...
def all_len(words:str):
    ...
    rueckgabe=[]
    for i in a:
        rueckgabe.append(len(a))
    return tuple(rueckgabe)
