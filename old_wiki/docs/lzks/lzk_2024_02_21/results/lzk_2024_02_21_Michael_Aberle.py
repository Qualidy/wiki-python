...
def resort_within_word(words):
    buchstaben = words.split()
    for i in buchstaben:
        buchstaben[buchstaben.index(i)] = i[::-1]
...
def keep_smallest_biggest(my_list:list):
    for i in my_list:
        if i == min(my_list) or i == max(my_list):
            my_list.append(i)
        else:
            my_list.remove(i)
    return my_list
...
def swap_digets(number:int):
    neu_zahlen = str(number)
    return int(neu_zahlen[::-1])

print(swap_digets(1000))
...
def get_unique_numbers(text:str):
    zahlen_sieben = (set())
    geschnittener_text = text.split(' ')
    for i in geschnittener_text:
        if i.isdigit():
            zahlen_sieben.add(int(i))
    return zahlen_sieben
