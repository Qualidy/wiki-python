...
# "2", 1 => None
def add_or_none(a, b):
    return a+b if (type(a) == int or type(a)) == float and (type(b) == int or type(b) == float) else None
...
def remove_non_vowel_starting_strings(string_list):
    return set([word for word in string_list if word[0].lower() in "aeiou"])
...
def squares_of_evens(input_list):
    return [zahl**2 for zahl in input_list if not zahl % 2]
...
def keys_with_string_values(my_dict):
    return sorted([key for key, value in my_dict.items() if type(value) == str])
...
def spoon(le):
    replacements = {
        'a': 'alawa', 'e': 'elewe', 'i': 'iliwi', 'o': 'olowo', 'u': 'uluwu',
        'ä': 'äläwä', 'ü': 'ülüwü', 'ö': 'ölöwö', 'ei': 'eileiwei',
        'A': 'Alawa', 'E': 'Elewe', 'I': 'Iliwi', 'O': 'Olowo', 'U': 'Uluwu',
        'Ä': 'ÄlÄwÄ', 'Ü': 'ÜlÜwÜ', 'Ö': 'ÖlÖwÖ', 'EI': 'Eileiwei'
    }
    result = le
