...
#
# Beispiel:
string_list= ['apple', 'banana', 'pear', 'orange'] #-> {'apple', 'orange'}
def remove_non_vowel_starting_strings(string_list):
    vocallist = ["a", "e", "i", "o", "u"]
    return set([element for element in string_list if element[0] in vocallist])
...
def squares_of_evens(input_list):
    return [element**2 for element in input_list if element%2 == 0]
...
def sort_by_length(strings):
    if len(strings[0])==len(strings[1])==len(strings[2]):
        return strings
    else:   
        return sorted(strings)
#print(sort_by_length(["aaa", "a", "aa"]))
...
def keys_with_string_values(my_dict):
    return [keys for keys, values in my_dict.items() if type(values)==str]
...
def zip_lists(list1, list2):
    ...
    if len(list1) <= len(list2):
        for index, element in enumerate(list1):
            result.append((element,list2[index]))
    else:
        for index, element in enumerate(list2):
            result.append((list1[index],element))
...
class Test_zip_lists(TestCase):
    ...
    def test_zip_lists_with_different_types(self):
        self.assertListEqual(zip_lists(["one", 2, 3.0], [True, False, "three"]), [("one", True), (2, False), (3.0, "three")])
...
def increment_numbers_in_string(input_string):
    return "".join([str(int(element)+1) if element.isnumeric() else element for element in input_string])
...
class Zahl:
    ...
    def __add__(self, other):
        if type(self) == type(other): #isinstance(self.zahl, other.zahl):
...
# -> Elewerbselewe
def spoon(le):
    translate_upper_dict = {"A" : "Alawa","E" : "Elewe", "I" : "Iliwi", "O" : "Olowo", "U" : "Uluwu", "Ä" : "Äläwä", "Ü" : "Ülüwü", "Ö" : "Ölöwö", "Ei" : "Eileiwei"}
    translate_dict = {"a" : "alawa","e" : "elewe", "i" : "iliwi", "o" : "olowo", "u" : "uluwu", "ä" : "äläwä", "ü" : "ülüwü", "ö" : "ölöwö", "ei" : "eileiwei"}
    le_list = [element for element in le]
    #print(le_list)
    
    for index, char in enumerate(le_list):
        if char == "e" and le_list[index+1]=="i" or char == "E" and le_list[index+1]=="i":
            le_list[index+1] = "ileiwei"

        else:
            if char in translate_dict.keys():
                le_list[index]=translate_dict[char]
            elif char in translate_upper_dict.keys():
                le_list[index]=translate_upper_dict[char]
    return "".join(le_list)
