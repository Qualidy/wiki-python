...
def sort_by_length(strings: list):
    for j in range(len(strings) - 1):
        for i in range(len(strings) - 1 - j):
...
def spoon(le):
    ...
    res = ''
    for i, char in enumerate(le, 0):
        if char == 'e' and i != len(le)-1 and le[i+1] == 'i':
            res += loeffelsprache['ei']
        elif char == 'i' and le[i-1] == 'e':
            continue
        elif char in loeffelsprache.keys():
            res += loeffelsprache[char]
        else:
            res += char
    return res
