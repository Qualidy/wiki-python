...
def remove_non_vowel_starting_strings(string_list):
    return set(string for string in string_list if string[0] in "aeiou")
...
def keys_with_string_values(my_dict):
    return sorted(key for key, value in my_dict.items() if isinstance(value, str))
...
def zip_lists(list1, list2):
    return [value for value in zip(list1, list2)]
...
def increment_numbers_in_string(input_string):
    return "".join(str(int(char) + 1) if char.isnumeric() else char for char in input_string)
...
# -> Elewerbselewe
def spoon(le):
    my_dict = {"a": "alawa", "o": "olowo", "u": "uluwu", "ä": "äläwä", "ü": "ülüwü",
               "ö": "ölöwö", "e": "elewe", "i": "iliwi", "A": "Alawa", "O": "Olowo",
               "U": "Uluwu", "Ä": "Äläwä", "Ü": "Ülüwü", "Ö": "Ölöwö", "E": "Elewe", "I": "Iliwi"}

    # for key, value in my_dict.items():
    #     if not key == "e" and not key == "i":
    #         le = le.replace(key, value)
    #         le = le.replace(key.upper(), value.capitalize())

    my_str = "".join(my_dict[le[i]] if le[i] in my_dict.keys() and le[i:i+2] != "ei" and le[i:i+2] != "Ei"
                                       and le[i-1:i+1] != "ei" and le[i-1:i+1] != "Ei"
                                        else le[i] for i in range(len(le)))
    my_str = my_str.replace("ei", "eileiwei")
    my_str = my_str.replace("Ei", "Eileiwei")
