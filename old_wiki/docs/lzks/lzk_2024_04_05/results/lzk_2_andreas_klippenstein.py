...
class Person:
    # input attributes here

    def vorstellung(self):
        print(f"Hallo Welt, ich bin {self.name} und ich bin {self.age} Jahre alt.")
...
# "2", 1 => None
def add_or_none(a, b):
    if not (type(a) is int) or not (type(a) is float):
        return None
    elif not (type(b) is int) or not (type(b) is float):
        return None
    else:
        return a + b
...
def remove_non_vowel_starting_strings(string_list):
    ...
    for counter, word in enumerate(string_list):
        if word[0] in vowels:
            tmp_set.add(word)
    tmp_set.remove("")
    return tmp_set
...
def sort_by_length(strings):
    tmp_list = []
    for counter, word in enumerate(strings):
        if len(word) > len(tmp_list[counter]):
            tmp_list.append(word)
        else:
            tmp_word = tmp_list[counter]
            word = tmp_list[counter]
            tmp_word[counter + 1] = tmp_word
...
def count_characters(s):
    ...
    for char in range(0, len(s)):
        tmp_dict[s[char]] = s.count(s[char])
    return tmp_dict
