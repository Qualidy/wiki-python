...
# "2", 1 => None
def add_or_none(a, b):
   test1 = isinstance(a, int) or isinstance(a, float)
   test2= isinstance(b, int) or isinstance(b, float)
   if test1 and test2:
       return a + b
   else:
       return None

print(add_or_none(0.2, 0.1))
...
def remove_non_vowel_starting_strings(string_list):
    return set([wort for wort in string_list if wort[0] in "aeiouüöäAEIOUÜÖÄ"])
...
def squares_of_evens(input_list):
    return [zahl**2 for zahl in input_list if zahl%2==0]
...
def sort_by_length(strings:list):
    if strings !=[] and strings !="" and strings !=None:
        laengen={}
        zergebnise=[None for _ in range(10)]
        for i in range(len(strings)):
            laengen[strings[i]]=len(strings[i])
        for key,val in laengen.items():
            i=val
            while zergebnise[i]!= None:
                i+=1
            zergebnise[i]=key

        ergbnis=[]
        for zergebnis in zergebnise:
            if zergebnis!=None or zergebnis=="":
                ergbnis.append(zergebnis)
...
def keys_with_string_values(my_dict):
    x= [val for _,val in my_dict.items()]
    x.sort()
    ergebnis=[]
    zero=False
    for wert in x:
...
def count_characters(s:str):
    ...
    s=s.lower()
    for zeichen in s:
        if zeichen in "qwertzuiopüasdfghjklöäyxcvbnmQWERTZUIOPÜASDFGHJKLÖÄYXCVBNM":
            ergebnis[f"{zeichen}"] = s.count(zeichen)
    return ergebnis
...
def increment_numbers_in_string(input_string):
    ...
    for zeichen in input_string:
        try:
            rechnen=int(zeichen)+1
            ergebnis+=str(rechnen)
        except:
            ergebnis+=zeichen
    return ergebnis
...
def spoon(le):
    if le!="":
        ...
        for index in range(laenge):
            zeichen=le[index].lower()
            if zeichen=="e" and le[index+1]=="i":
                index+=1
                ergebnis+=dict1["ei"]
            try:
                ergebnis+=dict1[le[index]]
            except:
                ergebnis+=le[index]
        return ergebnis
    else:
...
def valid_braces(la):
    ...
    for zeichen in la:
        if merkerklammernauf==0 and zeichen!="{":
            merkergueltig=False
            break
        elif zeichen=="}":
            merkerklammernzu+=1
            ...
            merkerklammernauf+=1
    if merkerklammernauf!=merkerklammernzu:
        merkergueltig=False
    return merkergueltig
