...
def remove_non_vowel_starting_strings(string_list: list[str]):
    return set([entry for entry in string_list if entry[0] in "aeiou"])
...
# Beispiel:
# [1, 2, 3, 4, 5] -> [4, 16]
def squares_of_evens(input_list: list[int]):
    return [num**2 for num in input_list if not num&1]
...
def keys_with_string_values(my_dict: dict):
    keys = my_dict.keys()
    result_list = [entry for entry in keys if isinstance(my_dict[entry], str)]
...
def count_characters(s: str):
    text = [entry for entry in s.lower() if entry.isalpha()]
    char_dict = dict(zip(set(text), [0] * len(set(text))) )
    for char in text:
        char_dict[char] += 1
    return char_dict
...
def zip_lists(list1, list2):
    ...
    return list(zip(list1[:min_len], list2[:min_len]))
...
def spoon(le: str):
    ...
    edit_text = le.lower().replace("ei", "1")
    post_edit_text = ""
    ...
    comp_str = le.split(" ")
    pre_corr_text = post_edit_text.split(" ")
    if len(le) > 0:
        return " ".join([comp_str[i][0] + pre_corr_text[i][1:] for i in range(len(comp_str))])
    else:
        return ""
