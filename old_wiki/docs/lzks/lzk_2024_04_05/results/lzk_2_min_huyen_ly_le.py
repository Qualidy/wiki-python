...
def remove_non_vowel_starting_strings(string_list):
    vocals = list('aeiou')
    return set([word for word in string_list if word[0] in vocals])
...
def squares_of_evens(input_list):
    return [number**2 for number in input_list if not number % 2]
...
def keys_with_string_values(my_dict):
    ...
    return (sorted(new_dict.keys()))
    #return my_dict.keys().sort()

keys_with_string_values({'a': 1, 'c': 'world', 'b': 'hello'})
...
def increment_numbers_in_string(input_string):
    list_of_chars = [str(int(elem) + 1) if elem.isdigit() else elem for elem in input_string]
...
class Zahl:
    def __init__(self, zahl):
        if isinstance(zahl, int|float):
            ...
            raise TypeError("Brauche Eine Zahl")

    def __add__(self, other):
        if isinstance(other, Zahl):
            return Zahl(self.zahl + other.zahl)
        else:
            raise NotImplementedError("Zahl nicht implementiert")
...
# -> Elewerbselewe
def spoon(le):
    vocals = ['ei', 'a', 'e', 'i', 'o', 'u', 'ä', 'ü', 'ö']
    vocals_upper = [v.title() for v in vocals]
    vocals = vocals + vocals_upper
    replace_vocals_dict = {v: f"{v}l{v.lower()}w{v.lower()}" for v in vocals}
    new_letters = []
    i = 0
    while i < len(le):
        if i+2 < len(le) and le[i: i+2] in vocals:
            new_letters.append(replace_vocals_dict[le[i: i+2]])
            i += 2
        elif le[i] in vocals:
            new_letters.append(replace_vocals_dict[le[i]])
            i += 1
        else:
            new_letters.append(le[i])
            i += 1
