...
def count_teachers() -> str:
    ...
    return """SELECT COUNT(lehrkraft.id)
                FROM lehrkraft
"""
...
FROM schuelerin
JOIN klasse ON schuelerin.klasse_id = klasse.id
WHERE klasse.name = ?"""
...
def exams_in_timeframe():
    ...
    return """SELECT COUNT(pruefung.id)
FROM pruefung
WHERE pruefung.typ = 'Ex' AND pruefung.datum >= ? AND pruefung.datum <= ?
"""
...
def all_klassensprecher():
    ...
    return """SELECT schuelerin.rufname, schuelerin.familienname FROM schuelerin WHERE schuelerin.ist_klassensprecher = '1'"""
...
def all_klassensprecher_sorted():
    ...
    return """SELECT schuelerin.rufname, schuelerin.familienname, klasse.name FROM schuelerin JOIN klasse ON schuelerin.klasse_id = klasse.id WHERE schuelerin.ist_klassensprecher = '1' ORDER BY schuelerin.familienname DESC"""
...
def youngest_student():
    ...
    Klasse zu erhalten."""
    return """SELECT schuelerin.rufname, schuelerin.familienname 
FROM schuelerin 
JOIN klasse ON schuelerin.klasse_id = klasse.id 
WHERE klasse.name = ? ORDER BY schuelerin.geburtsdatum ASC LIMIT 1"""
...
def average_value():
    ...
    return """SELECT AVG(note.wert)
FROM note
JOIN pruefung ON note.pruefung_id = pruefung.id
WHERE pruefung.id = ?"""
...
#     ('Hans', 'Gustav', 1.5)"""
#     return """
#     SELECT lehrkraft.rufname, lehrkraft.familienname, AVG(note.wert) AS Durchschnittsnote
#     FROM lehrkraft
#     JOIN note ON lehrkraft.id = note.lehrkraft_id
#     GROUP BY lehrkraft.rufname, lehrkraft.familienname
#     ORDER BY Durchschnittsnote ASC
#     """
#
# print(teacher_averages())
#
