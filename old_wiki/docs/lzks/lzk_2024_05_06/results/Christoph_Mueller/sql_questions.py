...
def count_teachers() -> str:
    ...
    return 'SELECT COUNT(lehrkraft.id) from lehrkraft'
...
def youngest_student():
    ...
    return '''SELECT schuelerin.rufname, schuelerin.familienname
                ...
                JOIN klasse ON schuelerin.klasse_id = klasse.id
                WHERE klasse.name like ?
                AND schuelerin.geburtsdatum like (SELECT MIN (geburtsdatum) from schuelerin)
                '''
...
def average_value():
    ...
    return ''' SELECT AVG(wert) FROM note Where pruefung_id like ?
    '''
