...
def size_of_class_sqlalchemy(class_name):
    ...
    return (session.query(func.count(Schuelerin.id))
            ...
            .filter(Klasse.name.like(f"{class_name}")).first()[0])
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return (session.query(func.count(Pruefung.id))
            .filter(Pruefung.datum.between({start_date},{end_date})).all())
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return (session.query(Schuelerin.rufname, Schuelerin.familienname, Klasse.name)
            ...
            .filter(Schuelerin.ist_klassensprecher == 1)
...
def youngest_student_sqlalchemy(class_name):
    ...
    return (session.query(Schuelerin.rufname, Schuelerin.familienname)
            .join(Klasse, Schuelerin.klasse_id == Klasse.id)
            .filter(Schuelerin.geburtsdatum == (session.query(func.max(Schuelerin.geburtsdatum))))
            )
