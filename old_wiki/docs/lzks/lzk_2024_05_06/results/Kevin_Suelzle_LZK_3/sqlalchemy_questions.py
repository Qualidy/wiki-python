...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return session.query(Pruefung).filter(Pruefung.datum.between(start_date, end_date)).filter(Pruefung.typ=='Ex').count()
...
def youngest_student_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname).join(Klasse).filter(Klasse.name == class_name).order_by(Schuelerin.geburtsdatum.asc()).first()
...
def average_value_sqlalchemy(exam_id):
    ...
    return session.query(Note.wert).filter(Note.pruefung_id == exam_id).all()
