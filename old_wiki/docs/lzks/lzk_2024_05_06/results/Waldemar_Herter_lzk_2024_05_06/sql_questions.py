...
def size_of_class() -> str:
    ...
    return  """ SELECT COUNT(schuelerin.id) FROM schuelerin
...
def youngest_student():
    ...
    Klasse zu erhalten."""
    return  """
            SELECT rufname, familkienname
            FROM schuelerin
            WHERE geburtsdatum < 2011-12-03
            AND klasse.name LIKE ?
            FIRST"""
...
def average_value():
    ...
    return  """
            SELECT AVG(wert)
            FROM note
            JOIN pruefung ON pruefung.id == note.pruefung_id
            WHERE pruefung.id LIKE ?"""
...
#     ('Hans', 'Gustav', 1.5)"""
#     return  """
#             SELECT lehrkraft.rufname, lehrkraft.familienname, AVG(wert)
#             FROM lehrkraft
#             JOIN note ON note.lehrkraft_id == lehrkraft.id
#             WHERE lehrkraft.rufname LIKE ?
#             AND lehrkraft familienname LIKE ?"""
#
