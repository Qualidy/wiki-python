...
def count_teachers_sqlalchemy():
    ...
    return session.query(func.count(Lehrkraft.id)).group_by(Lehrkraft.id).all()
...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(func.count(Schuelerin.id))\
            ...
            .filter(Klasse.name == class_name).all()
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    Beide Zeitpunkte sind inklusive. Es sollen nur Prüfungen vom Typ 'Ex' 
    betrachtet werden."""
    return session.query(func.count(Pruefung.id))\
        .filter(Pruefung.datum >= start_date)\
        .filter(Pruefung.datum <= end_date)\
        .filter(Pruefung.typ == 'Ex').all()
...
def all_klassensprecher_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname)\
        .filter(Schuelerin.ist_klassensprecher == 1).all()
...
def youngest_student_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname)\
        .filter(func.max(Schuelerin.geburtsdatum))\
        .join(Klasse, Klasse.id == Schuelerin.klasse_id)\
...
def average_value_sqlalchemy(exam_id):
    ...
    return session.query(func.count(Pruefung.fach_id)/(func.sum(Note.wert))\
        .join(Note, Note.fach_id == Pruefung.fach_id)\
        .filter(Pruefung.fach_id == exam_id)).all()
