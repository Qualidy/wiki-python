...
def count_teachers() -> str:
    ...
    return """
    SELECT count(lehrkraft.rufname) AS Anzahl_lehrkräfte
    FROM lehrkraft 
    """
...
def size_of_class(klasse) -> str:
    """⭐⭐Gibt den SQL-Ausdruck zurück, um die Anzahl der Schüler in einer Klasse zu erhalten."""
    return f"""
    SELECT count(schuelerin.familienname) AS Anzahl_schüler
    FROM schuelerin
    WHERE schuelerin.klasse_id == {klasse}
    """
...
def all_klassensprecher():
    ...
    FROM schuelerin
    WHERE schuelerin.ist_klassensprecher = True
    """
...
def all_klassensprecher_sorted():
    ...
    WHERE schuelerin.ist_klassensprecher = True
    ORDER BY schuelerin.rufname ASC, schuelerin.familienname DESC
    """
...
def youngest_student(klasse):
    ...
    WHERE klasse.name = '{klasse}' AND schuelerin.geburtsdatum = (
