...
def size_of_class() -> str:
    ...
    return """
    SELECT COUNT(*) FROM schuelerin WHERE klasse_id =?
    """
...
def exams_in_timeframe():
    ...
    return """
    SELECT COUNT(*) FROM pruefung WHERE pruefung.datum BETWEEN? AND? AND pruefung.typ = 'Ex'
    """
...
def all_klassensprecher_sorted():
    ...
    return """
    SELECT rufname, familienname, klasse_id FROM schuelerin WHERE ist_klassensprecher = 1 ORDER BY familienname
    """
...
def youngest_student():
    ...
    Klasse zu erhalten."""
    return """
    SELECT rufname ,familienname FROM schuelerin WHERE klasse_id =? ORDER BY geburtsdatum ASC LIMIT 1
    """
...
#     ('Hans', 'Gustav', 1.5)"""
#     return """
#     SELECT rufname, familienname, AVG(wert) FROM note WHERE lehrkraft_id=?
#     JOIN lehrkraft ON note.lehrkraft_id = lehrkraft.id GROUP BY lehrkraft.id ORDER BY AVG(wert) DESC
#     """
#
