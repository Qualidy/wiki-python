...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin).join(Klasse).filter(Klasse.name == class_name).count()
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return session.query(Pruefung).filter(Pruefung.datum >= start_date, Pruefung.datum <= end_date, Pruefung.typ == 'Ex').count()
...
def youngest_student_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname).join(Klasse).filter(Klasse.name == class_name).order_by(Schuelerin.geburtsdatum).limit(1)
...
def average_value_sqlalchemy(exam_id):
    ...
    return session.query(func.avg(Note.wert)).join(Pruefung).filter(Note.pruefung_id == exam_id).first()
