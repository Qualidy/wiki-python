...
def size_of_class() -> str:
    ...
    return """SELECT count(schuelerin.id) FROM schuelerin JOIN klasse ON schuelerin.klasse_id = klasse.id WHERE klasse.name = ?"""
...
def exams_in_timeframe():
    ...
    return """SELECT count(pruefung.id) FROM pruefung WHERE pruefung.datum between ? and ? and pruefung.typ LIKE 'EX'"""
...
def all_klassensprecher_sorted():
    ...
    return """
    SELECT schuelerin.rufname, schuelerin.familienname, klasse.name FROM schuelerin
...
def youngest_student():
    ...
    Klasse zu erhalten."""
    return """
    SELECT max(geburtsdatum), rufname, familienname
    FROM schuelerin
    JOIN klasse ON schuelerin.klasse_id = klasse.id
    WHERE klasse.name = ?
    GROUP BY klasse_id"""
...
#     ('Hans', 'Gustav', 1.5)"""
#     return """SELECT rufname, familienname, AVG(note.wert) AS Durchschnitt FROM lehrkraft JOIN note ON lehrkraft.id = note.lehrkraft_id GROUP BY familienname ORDER BY Durchschnitt ASC"""
#
