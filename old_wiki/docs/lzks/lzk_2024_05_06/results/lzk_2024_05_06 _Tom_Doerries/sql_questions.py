...
def count_teachers() -> str:
    ...
    return '''SELECT COUNT(lehrkraft.rufname)
                FROM lehrkraft
                '''
...
def size_of_class() -> str:
    ...
    return '''SELECT COUNT(schuelerin.familienname)
                FROM schuelerin
...
def exams_in_timeframe():
    ...
    return '''SELECT COUNT(pruefung.typ)
                FROM pruefung
                WHERE pruefung.typ LIKE "EX" AND pruefung.datum BETWEEN ? AND ?'''
...
#     ('Hans', 'Gustav', 1.5)"""
#     return '''SELECT lehrkraft.rufname, lehrkraft.familienname, AVG(note.wert)
#             FROM lehrkraft
