...
def count_teachers_sqlalchemy():
    ...
    return session.query(func.count(Lehrkraft.rufname)).scalar()
...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(func.count(Schuelerin.familienname)).join(Klasse, Schuelerin.klasse_id ==Klasse.id).filter(Klasse.name.like(class_name)).scalar()
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return session.query(func.count(Pruefung.typ)).filter(Pruefung.typ.like("EX")).filter(Pruefung.datum.between(start_date,end_date)).scalar()
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname,Schuelerin.familienname, Klasse.name).join(Klasse, Schuelerin.klasse_id == Klasse.id).filter(Schuelerin.ist_klassensprecher == 1).order_by(Schuelerin.familienname.desc()).all()
