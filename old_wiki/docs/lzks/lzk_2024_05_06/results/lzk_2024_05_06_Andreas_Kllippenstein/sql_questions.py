...
def all_klassensprecher_sorted():
    ...
    return '''
        SELECT rufname, 
        familienname, 
        (SELECT name FROM klasse WHERE id = klasse_id) 
        FROM schuelerin 
        WHERE ist_klassensprecher = 1 
...
#         SELECT rufname,
#         familienname,
#         (SELECT AVG(wert) FROM note)
#         FROM lehrkraft
#         WHERE id = (SELECT lehrkraft_id FROM note)
#         ORDER BY (SELECT AVG(wert) FROM note) ASC;
#     '''
#
