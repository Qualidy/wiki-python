...
def size_of_class() -> str:
    ...
    return "SELECT COUNT(*) FROM schuelerin WHERE klasse_id = (SELECT id FROM klasse WHERE name = ?)"
...
def exams_in_timeframe():
    ...
    return "SELECT COUNT(*) FROM pruefung WHERE typ = 'Ex' AND datum BETWEEN ? AND ?"
...
def all_klassensprecher_sorted():
    ...
    return "SELECT rufname, familienname, name FROM schuelerin JOIN klasse ON schuelerin.klasse_id = klasse.id WHERE ist_klassensprecher = 1 ORDER BY familienname DESC, name DESC"
...
def youngest_student():
    """⭐⭐⭐Gibt den SQL-Ausdruck zurück, um Ruf- und Familiennamen des jüngsten Schülers einer Klasse zu erhalten."""
    return "SELECT rufname, familienname FROM schuelerin WHERE klasse_id = (SELECT id FROM klasse WHERE name = ?) ORDER BY geburtsdatum ASC LIMIT 1"
