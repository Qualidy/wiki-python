...
def count_teachers_sqlalchemy():
    ...
    return session.query(func.count(Lehrkraft.id)).scalar()
...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(func.count(Schuelerin.id)).join(Klasse).filter(Klasse.name == class_name).scalar()
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return session.query(func.count(Pruefung.id)).filter(Pruefung.typ == 'Ex', Pruefung.datum.between(start_date, end_date)).scalar()
...
def all_klassensprecher_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname).filter(Schuelerin.ist_klassensprecher == True).all()
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname, Klasse.name).join(Klasse).filter(Schuelerin.ist_klassensprecher == True).order_by(desc(Schuelerin.familienname), desc(Klasse.name)).all()
...
def youngest_student_sqlalchemy(class_name):
    ...
    result = session.query(Schuelerin.rufname, Schuelerin.familienname).filter(Schuelerin.klasse.has(Klasse.name == class_name)).order_by(asc(Schuelerin.geburtsdatum)).first()
...
def average_value_sqlalchemy(exam_id):
    ...
    return session.query(func.avg(Note.wert)).filter(Note.pruefung_id == exam_id).scalar()
