...
# WHERE klasse.name LIKE ?
    return (
        session.query(func.count(Schuelerin.id))
...
#     WHERE ist_klassensprecher = 1
    return (
        session.query(Schuelerin.rufname, Schuelerin.familienname)
...
# SELECT rufname, familienname, klasse.name
# FROM schuelerin JOIN klasse ON schuelerin.klasse_id == klasse.id
# WHERE ist_klassensprecher = 1
# ORDER BY familienname DESC
    return (
        session.query(Schuelerin.rufname, Schuelerin.familienname, Klasse.name)
        .join(Klasse, Schuelerin.klasse_id==Klasse.id)
        .filter(Schuelerin.ist_klassensprecher==1)
...
# SELECT AVG(wert)
# FROM note
# WHERE pruefung_id LIKE ?
    return (
        session.query(func.avg(Note.wert))
        .filter_by(Note.pruefung_id == exam_id)
        .all()
    )
