...
def count_teachers_sqlalchemy():
    ...
    return"""
    session.query(func.count(Lehrkraft.lehrkraft_id)).scalar()
...
def size_of_class_sqlalchemy(class_name):
    ...
    return"""
    session.query(func.count(Schuelerin.id)).scalar()
    """
...
def all_klassensprecher_sqlalchemy():
    ...
    return """
    session.query(Schueler.rufname, Schueler.familienname).join(Klasse, Klasse.ist_klassensprecher == Schueler.schueler_id).all()
    """
