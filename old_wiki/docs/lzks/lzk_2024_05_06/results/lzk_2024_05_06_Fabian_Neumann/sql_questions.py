...
def exams_in_timeframe():
    ...
    select count(*) 
    from pruefung
    where datum between ? and ? and typ = 'Ex'
    """
...
def youngest_student():
    ...
    join klasse on schuelerin.klasse_id = klasse.id
    where klasse.name = ?1 
    and schuelerin.geburtsdatum = (select min(geburtsdatum) from schuelerin join klasse on schuelerin.klasse_id = klasse.id where klasse.name = ?1)
    """
...
def teacher_averages():
    ...
    ('Hans', 'Gustav', 1.5)"""
    return """
    select lehrkraft.rufname, lehrkraft.familienname, avg(note.wert)
    from lehrkraft
    join note on note.lehrkraft_id = lehrkraft.id
    join pruefung on note.pruefung_id = pruefung.id
    where pruefung.typ = 'Ex'
    group by lehrkraft.id
    order by avg(note.wert)
    """
