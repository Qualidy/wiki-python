...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return (session.query(Pruefung)
            .where(Pruefung.datum.between(start_date, end_date))
...
def youngest_student_sqlalchemy(class_name):
    ...
    min_geb = (session.query(Schuelerin.geburtsdatum)
                  .join(Klasse)
                  .where(Klasse.name == class_name)
    ...
    return (session.query(Schuelerin.rufname, Schuelerin.familienname)
            .join(Klasse)
            .where(Schuelerin.geburtsdatum == min_geb[0])
            .where(Klasse.name == class_name)
            .all())
...
def average_value_sqlalchemy(exam_id):
    ...
    grades = (session.query(Note.wert)
              .join(Pruefung)
              .where(Pruefung.id == exam_id)
              .all())

    avg = sum([int(grade[0]) for grade in grades]) / len(grades)
