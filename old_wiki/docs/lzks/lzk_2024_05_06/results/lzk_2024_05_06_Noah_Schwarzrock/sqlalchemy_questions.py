...
def count_teachers_sqlalchemy():
    ...
    return session.query(Lehrkraft.id).count()
...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin.id).join(Klasse).filter(Klasse.name.like(class_name)).count()
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return session.query(Pruefung.id).filter(Pruefung.datum.between(start_date, end_date), Pruefung.typ.like('Ex')).count()
...
def all_klassensprecher_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname).filter(Schuelerin.ist_klassensprecher==1).all()
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname, Klasse.name).join(Klasse).filter(Schuelerin.ist_klassensprecher == 1).order_by(Schuelerin.familienname.desc()).all()
...
def youngest_student_sqlalchemy(class_name):
    ...
    return [session.query(Schuelerin.rufname, Schuelerin.familienname).join(Klasse).filter(Klasse.name.like(class_name)).order_by(Schuelerin.geburtsdatum.asc()).first()]
...
def average_value_sqlalchemy(exam_id):
    ...
    return session.query(func.AVG(Note.wert)).join(Pruefung).filter(Pruefung.id == exam_id).all()
