...
def size_of_class() -> str:
    ...
    return """Select count(*) from schuelerin where klasse_id == 
              (Select id from klasse where name == ?);"""
...
def exams_in_timeframe():
    ...
    return """select count(*) from pruefung
...
def all_klassensprecher_sorted():
    ...
    return """select rufname, familienname, klasse.name from schuelerin 
              join klasse on schuelerin.klasse_id == klasse.id
              where ist_klassensprecher == 1
...
def youngest_student():
    ...
    return """select rufname, familienname from schuelerin 
              where schuelerin.geburtsdatum == 
              (Select min(geburtsdatum) from schuelerin join klasse on klasse.id == schuelerin.klasse_id 
              where klasse.name == ?);"""
...
#     ('Hans', 'Gustav', 1.5)"""
#     return """select rufname, familienname, avg(wert) as Durchschnittsnote
#               from note join lehrkraft on note.lehrkraft_id == lehrkraft.id
#               join pruefung on note.pruefung_id == pruefung.id
#               where typ == 'Ex'
#               group by note.lehrkraft_id
