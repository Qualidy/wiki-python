...
def size_of_class_sqlalchemy(class_name):
    ...
    return (session.query(Schuelerin).join(Klasse, Schuelerin.klasse_id == Klasse.id)
            .filter(Klasse.name == class_name).count())
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return session.query(Pruefung).filter(Pruefung.typ == "Ex" and start_date <= str(Pruefung.datum) <= end_date).count()
...
def all_klassensprecher_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname).filter(Schuelerin.ist_klassensprecher).all()
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return (session.query(Schuelerin.rufname, Schuelerin.familienname, Klasse.name)
            .join(Klasse, Klasse.id == Schuelerin.klasse_id)
            .filter(Schuelerin.ist_klassensprecher).order_by(Schuelerin.familienname.desc()).all())
...
def youngest_student_sqlalchemy(class_name):
    ...
    subquery = (session.query(func.min(Schuelerin.geburtsdatum))
                .join(Klasse, Klasse.id == Schuelerin.klasse_id)
                .filter(Klasse.name == class_name).first()[0])
    return (session.query(Schuelerin.rufname, Schuelerin.familienname).filter(Schuelerin.geburtsdatum == subquery)
            .all())
...
def average_value_sqlalchemy(exam_id):
    ...
    return session.query(func.avg(Note.wert)).filter(Note.pruefung_id == exam_id).first()[0]
