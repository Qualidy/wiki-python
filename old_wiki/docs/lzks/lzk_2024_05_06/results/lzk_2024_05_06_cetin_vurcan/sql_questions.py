...
def size_of_class() -> str:
    ...
    return '''select count(id) from schuelerin 
              WHERE klasse_id = (select id from Klasse WHERE name = ?)'''
...
def exams_in_timeframe():
    ...
    return '''SELECT count(id) from pruefung
              WHERE typ like 'Ex' AND datum BETWEEN ? AND ?'''
...
def all_klassensprecher():
    ...
    return '''select rufname,familienname from schuelerin
            where ist_klassensprecher like True'''
...
def all_klassensprecher_sorted():
    ...
    return '''select rufname, familienname, Klasse.name
              from schuelerin
              join Klasse ON schuelerin.klasse_id = Klasse.id
              where ist_klassensprecher = True
              ORDER BY familienname ASC, Klasse.name DESC'''
...
def youngest_student():
    ...
    return '''select schuelerin.rufname, schuelerin.familienname
              From schuelerin
              Join Klasse on schuelerin.klasse_id = (select klasse.id where klasse.name = ?)
...
#     ('Hans', 'Gustav', 1.5)"""
#     return '''select lehrkraft.rufname, lehrkraft.familienname, avg(Noten.wert)
#                 from lehrkraft
#                 Join Note on lehrkraft.id = Note.lehrkraft_id
#                     order by avg(Noten.wert) ASC
#               '''
#
