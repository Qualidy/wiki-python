...
def count_teachers_sqlalchemy():
    ...
    return session.query(func.count(Lehrkraft.id)).first()[0]
...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(func.count(Schuelerin.id)).join(Klasse).filter(Klasse.name==class_name).first()[0]
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return session.query(func.count(Pruefung.id).filter(Pruefung.datum.between(start_date, end_date), Pruefung.typ=='Ex')).all()
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname, Klasse.name).join(Klasse).order_by(Schuelerin.familienname).filter(Schuelerin.ist_klassensprecher==True).all()[::-1]
...
def youngest_student_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname).join(Klasse).filter(Klasse.name==class_name).all()
...
def average_value_sqlalchemy(exam_id):
    ...
    return session.query(Note.wert).filter(Note.pruefung_id==exam_id)
