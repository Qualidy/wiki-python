...
def size_of_class_sqlalchemy(class_name):
    ...
    klasse = session.query(Klasse).filter_by(name=class_name).first()
    if klasse:
        return session.query(Schuelerin).filter_by(klasse_id = klasse.id).count()
    else:
        return 0
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return session.query(Pruefung).filter(Pruefung.typ == 'Ex', Pruefung.datum.between(start_date, end_date)).count()
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname, Klasse.name).filter(Schuelerin.ist_klassensprecher == 1).join(Klasse, Schuelerin.klasse_id == Klasse.id).order_by(desc(Schuelerin.familienname)).all()
