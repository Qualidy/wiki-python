...
def size_of_class() -> str:
    ...
    return """
    select count(*) from schuelerin s left join klasse k ON s.klasse_id = k.id where k.name = ?
    """
...
def exams_in_timeframe():
    ...
    return """
    SELECT count(*) FROM pruefung WHERE datum >= ? AND datum <= ? AND typ LIKE 'Ex%';
    """
...
def all_klassensprecher():
    ...
    return """
    select rufname, familienname from schuelerin where ist_klassensprecher = 1
    """
...
def all_klassensprecher_sorted():
    ...
    return """
    select s.rufname, s.familienname, k.name from schuelerin s left join klasse k ON s.klasse_id = k.id where ist_klassensprecher = 1 ORDER BY familienname ASC
    """
...
def youngest_student():
    ...
    Klasse zu erhalten."""
    return """
    select s.rufname, s.familienname from schuelerin s left join klasse k ON s.klasse_id = k.id where k.name = ? AND s.geburtsdatum = (select MAX(geburtsdatum) from schuelerin)
    """
...
def average_value():
    ...
    return """
    select AVG(wert) from note where pruefung_id = ?
    """
...
#     ('Hans', 'Gustav', 1.5)"""
#     return """
#     select l.rufname, l.familienname, AVG(n.wert)
#     from note n LEFT JOIN lehrkraft l ON n.lehrkraft_id = l.id
#     """
#
