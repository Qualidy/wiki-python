...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin).join(Klasse).where(Klasse.name==class_name).count()
...
def youngest_student_sqlalchemy(class_name):
    ...
    maximum = max(n for n in session.query(Schuelerin.geburtsdatum).all())
    return session.query(Schuelerin.rufname, Schuelerin.familienname).join(Klasse).filter(Klasse.name==class_name, Schuelerin.geburtsdatum==maximum).first()
