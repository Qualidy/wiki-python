...
def exams_in_timeframe():
    ...
    return """SELECT COUNT(*)
            FROM pruefung
            WHERE pruefung.datum BETWEEN ? AND ? AND pruefung.typ = 'Ex'
            """
...
def all_klassensprecher_sorted():
    ...
    return """ SELECT schuelerin.rufname, schuelerin.familienname, klasse.name
              ...
              JOIN klasse ON schuelerin.klasse_id = klasse.id
              WHERE schuelerin.ist_klassensprecher = 1
              ORDER BY schuelerin.familienname ASC, klasse.name ASC
    """
...
def youngest_student():
    ...
    return """SELECT schuelerin.rufname, schuelerin.familienname
              ...
              JOIN klasse ON schuelerin.klasse_id = klasse.id
              WHERE klasse.name = ?
              ORDER BY schuelerin.geburtsdatum
              LIMIT 1
            """
...
#                FROM pruefung
#                JOIN note ON pruefung.id = note.pruefung_id
#                JOIN lehrkraft ON note.lehrkraft_id = lehrkraft.id
#                WHERE pruefung.id = ?
#            """
#
