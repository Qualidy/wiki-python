...
def count_teachers() -> str:
    ...
    return """SELECT SUM(1) FROM lehrkraft"""
...
def size_of_class() -> str:
    ...
    return """SELECT count(s.id) from klasse as k join schuelerin as s on s.klasse_id == k.id where k.name==? """
...
def exams_in_timeframe():
    ...
    return """select count(p.id) from pruefung as p  where p.typ =="Ex" and p.datum between ? and ?"""
...
def all_klassensprecher():
    ...
    return """select rufname, familienname from schuelerin as s where s.ist_klassensprecher ==1"""
...
def all_klassensprecher_sorted():
    ...
    return """select rufname, familienname, k.name from schuelerin as s join klasse as k on k.id==s.klasse_id where s.ist_klassensprecher ==1 order by s.familienname"""
...
def youngest_student():
    ...
    Klasse zu erhalten."""
    return """select rufname, familienname from schuelerin as s join klasse as k on k.id==s.klasse_id  where k.name == ? order by geburtsdatum asc limit 1"""
...
#     ('Hans', 'Gustav', 1.5)"""
#     return """select l.rufname, l.familienname, avg(n.wert) as avg_note
#     from lehrkraft as l
#     join  note as n  on  l.id==n.lehrkraft_id
#     group by l.rufname, l.familienname
...
# sql = """SELECT count(s.id) from klasse as k join schuelerin as s on s.klasse_id == k.id where k.name=="14b" """
# print(f"{pd.read_sql_query(sql, conn).to_markdown()}")
