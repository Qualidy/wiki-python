...
def count_teachers_sqlalchemy():
    ...
    return len(session.query(Lehrkraft).all())
...
def size_of_class_sqlalchemy(class_name):
    ...
    return len(session.query(Schuelerin).join(Klasse).where(Klasse.name==class_name).all())
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return  len(session.query(Pruefung).where(Pruefung.typ=="Ex").where(Pruefung.datum>=start_date).where(Pruefung.datum<=end_date).all())
...
def all_klassensprecher_sqlalchemy():
    ...
    res=  session.query(Schuelerin).where(Schuelerin.ist_klassensprecher == 1).all()
    resul=[(schuler.rufname, schuler.familienname) for schuler in res]
    return resul
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    res = session.query(Schuelerin.rufname,Schuelerin.familienname, Klasse.name).join(Klasse).where(Schuelerin.ist_klassensprecher == 1).order_by(Schuelerin.familienname.desc()).all()
    return [(w.rufname , w.familienname, w.name)for w in res]
...
def youngest_student_sqlalchemy(class_name):
    ...
    res = session.query(Schuelerin).join(Klasse).where(Klasse.name == class_name).order_by(Schuelerin.geburtsdatum).first()
    return [(res.rufname, res.familienname)] if res else []
...
def average_value_sqlalchemy(exam_id):
    ...
    res = session.query(Note).where(Note.pruefung_id==exam_id).all()
    return sum([w.wert for w in res])/len(res)
    return sum(session.query(Note).where(Note.pruefung_id==exam_id))/len(session.query(Note).where(Note.pruefung_id==exam_id))
