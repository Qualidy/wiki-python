...
def size_of_class() -> str:
    ...
    FROM schuelerin
    LEFT JOIN klasse ON schuelerin.klasse_id = klasse.id
...
def youngest_student():
    ...
    Klasse zu erhalten."""
    return """SELECT schuelerin.rufname, schuelerin.familienname, max(schuelerin.geburtsdatum) 
    FROM schuelerin
    LEFT JOIN klasse ON schuelerin.klasse_id = klasse.id
...
#     ('Hans', 'Gustav', 1.5)"""
#     return """SELECT lehrkraft.rufname AS rufname,
#     lehrkraft.familienname AS familienname,
#     avg(note.wert) AS durchschnitt
#     FROM lehrkraft
#     LEFT JOIN note ON lehrkraft.id = note.lehrkraft_id
#     WHERE lehrkraft.id == note.lehrkraft_id
#     GROUP BY rufname"""
#
#
