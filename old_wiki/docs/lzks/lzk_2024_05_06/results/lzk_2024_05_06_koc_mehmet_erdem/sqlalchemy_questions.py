...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(Klasse).join(Schuelerin).filter(Klasse.name == class_name).count()
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return session.query(Pruefung).filter(Pruefung.typ == 'Ex').filter(Pruefung.datum > start_date).filter(
        Pruefung.datum < end_date).count()
...
def average_value_sqlalchemy(exam_id):
    ...
    return session.get(Note, min(Note.wert)).filter(Note.pruefung_id == exam_id)
