...
def size_of_class() -> str:
    ...
    return "SELECT COUNT(schuelerin.id) FROM schuelerin JOIN klasse ON klasse.id = schuelerin.klasse_id WHERE klasse.name == '6a'"
...
def exams_in_timeframe():
    ...
    return "SELECT COUNT() FROM pruefung WHERE datum BETWEEN '2022-11-03' and '2022-11-24'"
...
def average_value():
    ...
    return "SELECT AVG(note.wert) FROM note JOIN pruefung ON note.pruefung_id = pruefung.id WHERE pruefung.id ==1 "
