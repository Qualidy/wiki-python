...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin).join(Klasse).where(Klasse.name == '6a').count()
...
def all_klassensprecher_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname,Schuelerin.familienname).where(Schuelerin.ist_klassensprecher==1).all()
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname,Schuelerin.familienname,Schuelerin.klasse_id).where(Schuelerin.ist_klassensprecher==1).order_by(Schuelerin.familienname.desc())
...
def youngest_student_sqlalchemy(class_name):
    ...
    return [session.query(Schuelerin.rufname,Schuelerin.familienname).join(Klasse).where(Klasse.name == '6a').order_by(Schuelerin.geburtsdatum).first()]
...
def average_value_sqlalchemy(exam_id):
    ...
    return session.query(Note.wert).join(Pruefung).filter(Pruefung.id.like(exam_id)).avg()
