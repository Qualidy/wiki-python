...
def size_of_class() -> str:
    ...
    return "SELECT COUNT(klasse_id) FROM schuelerin"
...
def all_klassensprecher():
    ...
    return "SELECT familienname, Rufname FROM schuelerin WHERE ist_klssensprecher = 1 ;"
...
def all_klassensprecher_sorted():
    ...
    return "SELECT familienname, Rufname, klasse_id FROM schuelerin WHERE ist_klssensprecher = 1 order by Familienname ASC; "
...
def youngest_student():
    ...
    Klasse zu erhalten."""
    return "SELECT rufname, familienname FROM schuelerin WHERE geburtsdatum = (SELECT MAX(geburtsdatum) FROM schuelerin)"
...
def average_value():
    ...
    return "SELECT AVG(wert) AS Durchschnittsnote FROM note WHERE pruefung_id = 1"
...
#     ('Hans', 'Gustav', 1.5)"""
#     return "SELECT rufname, famielienname, AVG(wert) AS Durchschnittsnote FROM note JOIN (SELECT rufname, famielienname FROM lehrkraft)"
#
