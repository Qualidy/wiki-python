...
def count_teachers_sqlalchemy():
    ...
    return session.query(function.count(Lehrkraft.id)).scalar()
...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(function.count(Schuelerin.familienname)).filter_by(Klasse.name).first()
...
def all_klassensprecher_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname,Schuelerin.familienname).filter(Schuelerin.ist_klassensprecher==1).order_by(Schuelerin.familienname.desc()).all()
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname,Schuelerin.familienname,Schuelerin.klasse_id).filter_by(Schuelerin.ist_klassensprecher==1).order_by(Schuelerin.familienname.desc)
...
def youngest_student_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin.rufname,Schuelerin.familienname).filter(function.MAX(Schuelerin.geburtsdatum))
...
def average_value_sqlalchemy(exam_id):
    ...
    return session.query(function.AVG(Note.wert)).filter(Note.lehrkraft_id)
