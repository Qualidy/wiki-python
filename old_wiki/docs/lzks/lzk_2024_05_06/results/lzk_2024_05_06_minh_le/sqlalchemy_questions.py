...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return (session.query(Pruefung)
            .filter(Pruefung.typ == 'Ex')
...
def all_klassensprecher_sqlalchemy():
    ...
    return (session.query(Schuelerin.rufname, Schuelerin.familienname)
            .where(Schuelerin.ist_klassensprecher).all())
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return (session.query(Schuelerin.rufname, Schuelerin.familienname, Klasse.name).join(Klasse).where(Schuelerin.ist_klassensprecher)
            .order_by(Schuelerin.familienname.desc()).all())
...
def youngest_student_sqlalchemy(class_name):
    ...
    return (session.query(Schuelerin.rufname, Schuelerin.familienname)
            ...
            .group_by(Schuelerin.id)
            .order_by(Schuelerin.geburtsdatum)
            .limit(1).all())


class TestYoungestStudentSqlalchemy(TestCase):
    def test_youngest_student_0(self):
        print(youngest_student_sqlalchemy('6a'))
...
def average_value_sqlalchemy(exam_id):
    ...
    return session.query(func.avg(Note.wert)).where(Note.pruefung_id==exam_id).scalar()
