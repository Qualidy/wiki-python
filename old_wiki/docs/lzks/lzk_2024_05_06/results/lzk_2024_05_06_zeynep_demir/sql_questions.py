...
def size_of_class() -> str:
    ...
    return """SELECT COUNT(*) FROM schuelerin JOIN klasse ON schuelerin.klasse_id = klasse.id WHERE klasse.name LIKE ?"""
...
def exams_in_timeframe():
    ...
    return """SELECT COUNT(*) FROM pruefung WHERE typ LIKE 'ex' AND datum >=? AND datum <=?"""
...
def all_klassensprecher_sorted():
    ...
    return """SELECT schuelerin.rufname, schuelerin.familienname, klasse.name FROM schuelerin JOIN klasse ON klasse.id = schuelerin.klasse_id  WHERE schuelerin.ist_klassensprecher = 1 ORDER BY schuelerin.familienname DESC, klasse.name ASC"""
...
def youngest_student():
    ...
    Klasse zu erhalten."""
    return """SELECT schuelerin.rufname, schuelerin.familienname FROM schuelerin JOIN klasse On klasse.id = schuelerin.klasse_id WHERE klasse.name LIKE ? ORDER BY schuelerin.geburtsdatum ASC LIMIT 1"""
...
def average_value():
    ...
    return """SELECT AVG(note.wert) FROM note JOIN pruefung ON pruefung.id = note.pruefung_id WHERE pruefung.id LIKE ?"""
