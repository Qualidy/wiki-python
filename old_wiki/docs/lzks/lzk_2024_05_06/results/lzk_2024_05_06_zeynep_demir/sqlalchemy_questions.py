...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin.id).join(Klasse).filter(Klasse.name == class_name).count()
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return (session.query(Pruefung.id).filter(Pruefung.typ == "Ex", Pruefung.datum >= start_date, Pruefung.datum <= end_date)).count()
...
def youngest_student_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname).join(Klasse).filter(Klasse.name == class_name).order_by(Schuelerin.geburtsdatum.asc()).limit(1)
