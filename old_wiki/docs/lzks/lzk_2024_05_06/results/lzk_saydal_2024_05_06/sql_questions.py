...
def count_teachers() -> str:
    ...
    return """ SELECT count(lehrkraft.id) AS Anzahl_Lehrer 
    FROM lehrkraft
    """
...
def size_of_class() -> str:
    ...
    return """SELECT klasse.name, COUNT(schuelerin.id) AS Anzahl_Schüler_in_Klasse 
    FROM klasse
    JOIN schuelerin ON schuelerin.klasse_id = klasse.id
...
def exams_in_timeframe():
    ...
    return """ SELECT COUNT(pruefung.id) AS Anzahl_Pruefungen_in_Zeitfenster 
    FROM pruefung
    WHERE pruefung.typ LIKE 'EX'
    AND pruefung.datum >= '%%%%-%%-%%'
    AND pruefung.datum <= '%%%%-%%-%%'
    """
...
def all_klassensprecher():
    ...
    WHERE schuelerin.ist_klassensprecher IS NOT 0
    """
...
def all_klassensprecher_sorted():
    ...
    return """SELECT schuelerin.familienname, schuelerin.rufname, klasse.name AS Klasse
    FROM schuelerin
    JOIN klasse ON schuelerin.klasse_id = klasse.id
    WHERE schuelerin.ist_klassensprecher IS NOT 0
    ORDER BY schuelerin.familienname ASC
    """
...
def average_value():
    ...
    return """ SELECT AVG(note.wert) AS Durchschnittsnote FROM note """
