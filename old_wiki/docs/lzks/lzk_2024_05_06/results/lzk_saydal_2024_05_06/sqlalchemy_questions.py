...
def count_teachers_sqlalchemy():
    ...
    return session.query(func.count(Lehrkraft.id)).all()
print('Anzahl der vorhandenen Lehrkräfte:', count_teachers_sqlalchemy())
...
def size_of_class_sqlalchemy(class_name):
    ...
    return session.query(Schuelerin).filter_by(klassename=class_name).count()
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return session.query(func.count(Pruefung.id)).filter(Pruefung.typ == 'Ex')
...
def all_klassensprecher_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname).filter_by(ist_klassensprecher=True).all()
print(*all_klassensprecher_sqlalchemy(), sep='\n')
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname, Schuelerin.klasse_id).filter_by(ist_klassensprecher=True).order_by(Schuelerin.familienname.desc()).all()
