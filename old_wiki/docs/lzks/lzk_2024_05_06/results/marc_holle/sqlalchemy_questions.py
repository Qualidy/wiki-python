...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
  ...
  .filter(Pruefung.typ.like('EX%')) \
  .filter(Pruefung.datum.between(start_date, end_date)) \
  .scalar()
...
def youngest_student_sqlalchemy(class_name):
    ...
    return session.query(func.max(Schuelerin.geburtsdatum), Schuelerin.rufname, Schuelerin.familienname) \
  .join(Klasse, Schuelerin.klasse_id == Klasse.id) \
  .filter(Klasse.name == "?") \
  .group_by(Klasse.id)
