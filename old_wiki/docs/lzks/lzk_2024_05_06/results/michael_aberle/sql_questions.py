...
def size_of_class() -> str:
    ...
    return '''SELECT count(name) FROM klasse Join schuelerin on klasse.id == schuelerin.klasse_id WHERE name = ? '''
...
def exams_in_timeframe():
    ...
    return '''SELECT count(datum) FROM pruefung WHERE datum >= ? and datum <= ? '''
...
def all_klassensprecher_sorted():
    ...
    return '''SELECT rufname , familienname FROM schuelerin WHERE ist_klassensprecher'''
...
def youngest_student():
    ...
    Klasse zu erhalten."""
    return '''SELECT rufname, familienname FROM schuelerin JOIN klasse on klasse.id == schuelerin.klasse_id WHERE name = ? and max(geburtsdatum)'''
