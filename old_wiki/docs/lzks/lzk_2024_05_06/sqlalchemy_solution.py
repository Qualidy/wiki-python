...
def count_teachers_sqlalchemy():
    ...
    return session.query(Lehrkraft.id).all().__len__()
...
def size_of_class_sqlalchemy(class_name):
    ...
    return len(session.query(Schuelerin.id)
               .join(Klasse).filter(Klasse.name == class_name).all())
...
def exams_in_timeframe_sqlalchemy(start_date, end_date):
    ...
    return (session.query(Pruefung.id)
            .filter(Pruefung.datum >= start_date, Pruefung.datum <= end_date, Pruefung.typ == 'Ex')
            .all().__len__())
...
def all_klassensprecher_sqlalchemy():
    ...
    return (session.query(Schuelerin.rufname, Schuelerin.familienname)
            .filter(Schuelerin.ist_klassensprecher == True).all())
...
def all_klassensprecher_sorted_sqlalchemy():
    ...
    return session.query(Schuelerin.rufname, Schuelerin.familienname, Klasse.name) \
        .join(Klasse) \
        .filter(Schuelerin.ist_klassensprecher == True) \
        .order_by(Schuelerin.familienname.desc()) \
        .all()
...
def youngest_student_sqlalchemy(class_name):
    ...
    result = session.query(Schuelerin.geburtsdatum) \
        .join(Klasse) \
        .filter(Klasse.name == class_name) \
        .order_by(Schuelerin.geburtsdatum.asc()) \
        .first()

    if result is None:
        return []

    youngest_date = result[0]

    return session.query(Schuelerin.rufname, Schuelerin.familienname) \
        .filter(Schuelerin.geburtsdatum == youngest_date) \
        .all()
...
def average_value_sqlalchemy(exam_id):
    ...
    alle_noten = session.query(Note.wert).filter(Note.pruefung_id == exam_id).all()
    alle_noten = [note[0] for note in alle_noten]
